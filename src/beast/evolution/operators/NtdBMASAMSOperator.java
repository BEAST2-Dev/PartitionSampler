 ...
 * @author Chieh-Hsi Wu
 */
public class NtdBMASAMSOperator extends Operator {
    ...
    public Input<DirichletDistribution> freqsDistrInput = new Input<DirichletDistribution>(
            "freqsDistr",
    ...
    public double split(int index1, int index2, int clusterIndex, int[] initClusterSites){
        try{
            ...
            QuietRealParameter newFreqs = new QuietRealParameter(new Double[4]);
            logqSplit+=proposeNewFreqValues(
                    newFreqs,
                    freqsList.getValues(clusterIndex),
                    freqsBaseDistr,
                    freqsList.getUpper(),
                    freqsList.getLower()
            );
            ...
            for(int i = 0;i < clusterSites.length; i++){
                ...
                if(draw < newClusterProb){
                    ...
                    /*if(Double.isNaN(logqSplit)){
                        throw new RuntimeException(""+newClusterProb);
                    }*/
                    cluster1Count++;
                }else{
