...
public class RateSAMSOperator extends Operator {
    ...
    public Input<ParametricDistribution> distrInput = new Input<ParametricDistribution>(
            "rateDistr",
            "The proposal distribution for rate.",
            Input.Validate.REQUIRED
    );
    ...
    public double split(int index1, int index2, int clusterIndex, int[] initClusterSites){
        try{
            ...
            //QuietRealParameter newParam = getSample(baseDistr, rateList.getUpper(), rateList.getLower());
            QuietRealParameter newParam = new QuietRealParameter(new Double[1]);
            logqSplit += proposalValueInLogSpace(newParam, rateList.getValue(clusterIndex,0), baseDistr,rateList.getUpper(), rateList.getLower());
            ...
            //logqSplit += baseDistr.calcLogP(newParam);
    ...
    public double merge(
        ...
        try{
            ...
            logqMerge += mergeValueInLogSpace(
                    rateList.getParameter(clusterIndex1),
                    rateList.getParameter(clusterIndex2),
                    baseDistr);
        }catch(Exception e){
