...
import beast.evolution.likelihood.GeneralUnitTempTreeLikelihood;
 ...
 * @author Chieh-Hsi Wu
 */
public class NtdBMAGammaBMADPPGibbsSampler extends Operator implements Loggable {
    ...
    public Input<ParametricDistribution> paramBaseDistrInput = new Input<ParametricDistribution>(
            "paramBaseDistr",
            "The base distribution of the infinitesimal rate matrix parameters of nucleotide substitution model",
            Input.Validate.REQUIRED
    );

    public Input<ParametricDistribution> modelBaseDistrInput = new Input<ParametricDistribution>(
            "modelBaseDistr",
            "The base distribution of the nucleotide substitution model",
            Input.Validate.REQUIRED
    );

    public Input<ParametricDistribution> freqsBaseDistrInput = new Input<ParametricDistribution>(
            "freqsBaseDistr",
            "The base distribution of the nucleotide frequencies",
            Input.Validate.REQUIRED
    );
    ...
    public Input<GeneralUnitTempTreeLikelihood> tempLikelihoodInput = new Input<GeneralUnitTempTreeLikelihood>(
            "tempLikelihood",
    ...
    public void initAndValidate(){
        ...
        freqsBaseDistr = freqsBaseDistrInput.get();
    ...
    public double proposal(){
        ...
        //RealParameter currAlphaVal = alphaList.getParameter(listIndex);
        //RealParameter currInvPrVal = invPrList.getParameter(listIndex);
        //RealParameter currSiteModel = siteModelList.getParameter(listIndex);




        GeneralUnitTempTreeLikelihood tempLik = tempLikelihoodInput.get();
        ...
        try{
            ...
            QuietRealParameter[] paramPreProposals = getSamples(paramBaseDistr, paramList.getUpper(), paramList.getLower());
            QuietRealParameter[] modelPreProposals = getSamples(modelBaseDistr, modelList.getUpper(), modelList.getLower());
            QuietRealParameter[] freqsPreProposals = getSamples(freqsBaseDistr, freqsList.getUpper(), freqsList.getLower());
            ...
            for(; i < logFullCond.length; i++){
                ...
                logFullCond[i] = logFullCond[i]+tempLik.calculateLogP(
                        ...
                        modelPreProposals[i-counter],
                        freqsPreProposals[i-counter],
                        alphaPreProposals[i-counter],
                        invPrPreProposals[i-counter],
                        ratesPreProposals[i-counter],
                        siteModelPreProposals[i-counter],
                        index
                );
            ...
            if(curr != freqsProposal){//Takes a different value from the current
                ...
                if(proposedIndex >= counter && zeroCount > -1){//Singleton takes new value
                    ...
                    alphaList.setValue(paramListIndex,0,alphaProposal.getValue());
                    invPrList.setValue(paramListIndex,0,invPrProposal.getValue());
                    ratesList.setValue(paramListIndex,0,ratesProposal.getValue());
                    siteModelList.setValue(paramListIndex,0,siteModelProposal.getValue());
                    zeroCount = -1;
    ...
    public double getSiteLogLikelihood(
            ...
            RealParameter invPr,
            RealParameter rates,
            RealParameter siteModel,
            int clusterIndex,
            int siteIndex,
            GeneralUnitTempTreeLikelihood tempLik){
