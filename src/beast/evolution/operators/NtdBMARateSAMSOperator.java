 ...
 * @author Chieh-Hsi Wu
 */
public class NtdBMARateSAMSOperator extends Operator {
    ...
    public Input<ParametricDistribution> rateDistrInput = new Input<ParametricDistribution>(
            "rateDistr",
            "Sampling distribution for relative rates.",
            Input.Validate.REQUIRED
    );
    ...
    public double split(int index1, int index2, int clusterIndex, int[] initClusterSites){
        try{
            ...
            //logqSplit += proposeNewValue(newParam, paramBaseDistr, paramList.getUpper(), paramList.getLower());
            ...
            logqSplit += proposeNewValue2(newParam, oldParamValues, paramBaseDistr, paramList.getUpper(), paramList.getLower());
            ...
            QuietRealParameter newModel = new QuietRealParameter(new Double[1]);
             logqSplit += proposeDiscreteValue(newModel, modelList.getValue(clusterIndex,0), modelDistrInput.get(), modelList.getUpper(),modelList.getLower());
            ...
            logqSplit+=proposalValueInLogSpace(newRates,ratesList.getValue(clusterIndex, 0), ratesBaseDistr, ratesList.getUpper(), ratesList.getLower());
    ...
    private double proposeNewValue(QuietRealParameter proposal, ParametricDistribution distr, double upper, double lower) throws Exception{
    ...
    private double proposeNewValue2(QuietRealParameter proposal, double[] oldValues, ParametricDistribution distr, double upper, double lower) throws Exception{
        Double[] sampleVals = distr.sample(1)[0];
        for(int i = 0; i < sampleVals.length; i++){
    ...
    public double proposalValueInLogSpace(QuietRealParameter proposal, double oldValue, ParametricDistribution distr, double upper, double lower) throws Exception{
